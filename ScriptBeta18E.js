...
function handleTouchEnd(e) {
    if (gameState === 'playing' && touchMoveActive && lastTouchX !== null && lastTouchY !== null) {
        // Shoot on touch end
        const playerBot = bots.find(bot => bot.isPlayer);
        const angle = Math.atan2(lastTouchY - playerBot.y, lastTouchX - playerBot.x);
        createProjectile(playerBot.x, playerBot.y, angle, 'player');
...
// In your update loop, move player toward last touch position if touchMoveActive
function movePlayerWithTouch() {
    if (gameState !== 'playing' || !touchMoveActive || lastTouchX === null || lastTouchY === null) return;
    const playerBot = bots.find(bot => bot.isPlayer);
    const angle = Math.atan2(lastTouchY - playerBot.y, lastTouchX - playerBot.x);
    const targetX = playerBot.x + Math.cos(angle) * playerSpeed;
    const targetY = playerBot.y + Math.sin(angle) * playerSpeed;
...
// Call movePlayerWithTouch() in your update loop before drawing the player
function update() {
    // Draw floor texture first
    if (floorPattern) {
        ctx.save();
