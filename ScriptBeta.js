...
function initializeBots() {
    ...
    if (selectedCharacter) {
        if (selectedCharacter.name === 'Colt') {
            botHealth = 2000; // Bot health when playing with Colt
        } else if (selectedCharacter.name === 'El Primo') {
            botHealth = 3000; // Bot health when playing with El Primo
        } else if (selectedCharacter.name === 'Jessie') {
            botHealth = 6000; // Bot health when playing with Jessie
        }
    }
...
canvas.addEventListener('click', (event) => {
    if (gameState === 'playing') {
        ...
        const angle = Math.atan2(mouseY - playerBot.y, mouseX - playerBot.x);
        createProjectile(playerBot.x, playerBot.y, angle, 'player'); // Pass source as 'player'
...
function checkGameOver() {
    ...
    if (bots.filter(bot => !bot.isPlayer).length === 0) {
        ...
        if (!elPrimoUnlocked) {
            // Unlock El Primo
            characters.push({ name: 'El Primo', color: 'darkgreen', damage: 30, health: 6300 });
            elPrimoUnlocked = true;
        } else {
            // Unlock Jessie
            const elPrimo = characters.find(char => char.name === 'El Primo');
            if (selectedCharacter === elPrimo && !jessieUnlocked) {
                characters.push({ name: 'Jessie', color: 'orange', damage: 25, health: 3100 });
                jessieUnlocked = true;
            }
        }
    }
...
function update() {
    ...
    if (gameState === 'home') {
        ...
        // Check game over - Moved to the end of the playing state
        checkGameOver();
        ...
        // Display health
        const playerBot = bots.find(bot => bot.isPlayer);
         // Ensure playerBot exists before accessing its properties
        if (playerBot) {
            ctx.fillStyle = 'white';
            ctx.font = '16px Arial';
            ctx.textAlign = 'left';
            ctx.fillText('Health: ' + Math.max(0, playerBot.health), 10, 40); // Prevent negative health display
        }
    } else if (gameState === 'gameOver') {
        ...
        // Draw "Quit and Go to Menu" button
        ctx.font = '20px Arial';
        ...
        ctx.fillStyle = 'rgba(0, 0, 0, 0.7)';
        ctx.fillRect(0, 0, canvas.width, canvas.height);
        drawVictoryText(); // Call the static victory text function
    }
