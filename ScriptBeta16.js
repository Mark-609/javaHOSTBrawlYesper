...
bushImage.src = "https://i.postimg.cc/0NcVM9tj/bush-texture.png";
...
wallImage.src = "https://i.postimg.cc/Bn5Y3THx/wall-texture.png";
...
floorImage.src = "https://i.postimg.cc/MpJPgPM6/floor-texture.png";
...
stoneImage.src = "https://i.postimg.cc/CKFvxRs9/stone-deco.png";
...
function initializeBots() {
    ...
    // Helper to check if a position is inside any wall
    function isInWall(x, y, radius) {
    ...
    // Spawn bots (not player) outside walls
    for (let i = 0; i < numBots - 1; i++) {
        let x, y, tries = 0;
        do {
            x = randomIntFromRange(50, canvas.width - 50);
            y = randomIntFromRange(50, canvas.height - 50);
            tries++;
        } while (isInWall(x, y, botRadius) && tries < 100);
        createBot(x, y, botHealth);
    }

    // Spawn player outside walls
    let px, py, pTries = 0;
    do {
        px = canvas.width / 2;
        py = canvas.height / 2;
        pTries++;
        // If center is in wall, try random positions
        if (isInWall(px, py, playerRadius)) {
            px = randomIntFromRange(50, canvas.width - 50);
            py = randomIntFromRange(50, canvas.height - 50);
        }
    } while (isInWall(px, py, playerRadius) && pTries < 100);
...
function moveBots() {
    bots.forEach(bot => {
        if (!bot.isPlayer) {
            ...
            if (canMoveX && targetX > bot.radius && targetX < canvas.width - bot.radius) {
                bot.x = targetX;
            }
            if (canMoveY && targetY > bot.radius && targetY < canvas.height - bot.radius) {
                bot.y = targetY;
            }
...
menuBgImage.src = "https://i.postimg.cc/nrPrSSHp/background-texture.jpg";
